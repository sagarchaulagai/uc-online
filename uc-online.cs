...
namespace uc_online
{
    public class uc_online : IDisposable
    {
        ...
        public uc_online(string iniFilePath = "config.ini")
        {
            ...
            _logger.Log($"uc-online initialized with AppID: {_currentAppID}");
        ...
        private void LoadSteamApiDll()
        {
            try
            {
                ...
                string dllName = is64Bit ? "steam_api64.dll" : "steam_api.dll";

                _logger.Log($"Current process is {(is64Bit ? "64-bit" : "32-bit")}, looking for {dllName}");
                ...
                string dllPath = Path.Combine(_steamApiDllPath, dllName);
                if (File.Exists(dllPath))
                {
                    _logger.Log($"Found {dllName} at: {dllPath}");

                    IntPtr handle = LoadLibrary(dllPath);
                    if (handle != IntPtr.Zero)
                    {
                        _logger.Log($"Successfully loaded {dllName} from custom path");
                    }
                    else
                    {
                        _logger.LogWarning($"Failed to load {dllName} from custom path, falling back to default loading");
                    }
                }
                else
                {
                    if (is64Bit)
                    {
                        string win64Path = Path.Combine(_steamApiDllPath, "win64", dllName);
                        if (File.Exists(win64Path))
                        {
                            _logger.Log($"Found {dllName} at: {win64Path}");
